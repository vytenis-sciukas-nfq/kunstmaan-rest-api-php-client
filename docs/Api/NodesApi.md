...
```php
getNodeChildren($id, $page, $limit): \NFQ\KunstmaanRestApi\PhpClient\Model\PaginatedNodeList
```
...
try {
    $result = $apiInstance->getNodeChildren($id, $page, $limit);
    print_r($result);
} catch (Exception $e) {
...
```php
getNodes($page, $limit, $internalName, $locale, $hiddenFromNav, $refEntityName): \NFQ\KunstmaanRestApi\PhpClient\Model\PaginatedNodeList
```
